...
void resolution2_GX(int idx=24, int det = 3, float cutsingle=0.90, float cutmultiple=0.8 )
{
  fIndex = idx;
  fDet = det;
  GetConfig();
  ...
  TString cuts2chargesharingMultiple = Form("%s&&((qmampl%s/ampl%s)<%f)",cuts2.Data(),fSDet.Data(),fSDet.Data(),cutmultiple);
  TString cuts2chargesharingSingle = Form("%s&&((qmampl%s/ampl%s)>%f)",cuts2.Data(),fSDet.Data(),fSDet.Data(),cutsingle);
  ...
  //
  tree->Draw( Form("(qmampl%s/ampl%s):bx%s>>histMaAvsBX(%d,%f,%f,%d,%f,%f)",
		   fSDet.Data(),fSDet.Data(),fSDet.Data(),
		   2*nbinsX,fGXmin,fGXmax,50,0.0,1.0),
	      cuts2.Data(), "col");
  ...
  publish->SaveAs( Form("res/D%dGX/DXBXP_D%d_%d.pdf",det,det,fRun), "pdf");
  //=====
  main2->cd(5);
  DXMBX->Draw("col");
  GXvsBX->Draw("col");
  ...
  histDXMBY->Draw("col");
  publish->SaveAs( Form("res/D%dGX/DXBYP_D%d_%d.pdf",det,det,fRun), "pdf");
  //=====
  main2->cd(9);
  histDXMBY->Draw("col");
  //=====
  ...
  ofstream fcorout;
  // CORR-X
  fcorout.open( Form("res/D%dGX/CORXY/CORX_D%d_%d.dat",det,det,fRun) );
  ...
  fcorout.close();
  // CORR-XY
  fcorout.open( Form("res/D%dGX/CORXY/CORXY_D%d_%d.dat",det,det,fRun) );
  ...
  publish->SaveAs( Form("res/D%dGX/DX3_D%d_%d.pdf",det,det,fRun), "pdf");
  hist19->Draw();
  hist19->SetTitle("");
  ...
  publish->SaveAs( Form("res/D%dGX/DX1_D%d_%d.pdf",det,det,fRun), "pdf");
  ...
  */
  //=====
  main1->SaveAs( Form("res/D%dGX/resolution_%s_%s_%s_%d.pdf[",
  ...
  main1->SaveAs( Form("res/D%dGX/resolution_%s_%s_%s_%d.pdf",
		     det,fBoard.Data(),fSDet.Data(),fCell.Data(),fRun),
		"pdf");
  main2->SaveAs( Form("res/D%dGX/resolution_%s_%s_%s_%d.pdf",
		     det,fBoard.Data(),fSDet.Data(),fCell.Data(),fRun),
		"pdf");
  ...
  tex->DrawTextNDC(0.6,0.85,Form("Entries %.1f k",dxSingle->GetEntries()*1e-3));
  tex->DrawTextNDC(0.6,0.8,Form("RMS %.1f um",dxSingle->GetRMS()*1e3));
  publish->SaveAs( Form("res/D%dGX/singleDXcounts_D%d_%d.pdf",det,det,fRun), "pdf");
  main1->cd(7);
  ...
  tex->DrawTextNDC(0.6,0.85,Form("Entries %.1f k",dxMultiple->GetEntries()*1e-3));
  tex->DrawTextNDC(0.6,0.8,Form("RMS %.1f um",dxMultiple->GetRMS()*1e3));
  publish->SaveAs( Form("res/D%dGX/multipleDXcounts_D%d_%d.pdf",det,det,fRun), "pdf");
  ...
  main2->SaveAs( Form("res/D%dGX/FIDUCIAL_D%d_%d.pdf",
		      det,det,fRun),
		"pdf");
  //======
  //======
  ...
  main3->SaveAs( Form("res/D%dGX/resolution_%s_%s_%s_%d.pdf",
		      det,fBoard.Data(),fSDet.Data(),fCell.Data(),fRun),
		 "pdf");
  main3->SaveAs( Form("res/D%dGX/resolution_%s_%s_%s_%d.pdf]",
		      det,fBoard.Data(),fSDet.Data(),fCell.Data(),fRun),
  ...
  pMAXvsBX->Draw("same");
  mainExtra->SaveAs(Form("res/D%dGX/qaextra_%s_%s_%s_%d.pdf",
			 det,fBoard.Data(),fSDet.Data(),fCell.Data(),fRun),
		    "pdf");
  ...
  ofstream fout( Form("res/D%dGX/data/reso_%s_%s_run%d.dat",
		      det,fTech.Data(),fBoard.Data(),fRun) );
  std::cout << " RESULTS IN " << Form("res/D%dGX/data/reso_%s_%s_run%d.dat",

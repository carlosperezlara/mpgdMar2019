...
bool filterKeyRuns(int run) {
	if(run==1513) return true; // 0% 2.00
	if(run==1514) return true; // 0% 0.83
	if(run==1375) return true; // 5% 0.67
	if(run==1379) return true; // 5% 0.40
	if(run==1369) return true; // 5% 0.67
	if(run==1511) return true; // 5% 0.67
	if(run==1445) return true; // 5% 1.00
	if(run==1456) return true; // 5% 2.00
	if(run==1399) return true; // st 1.00
	if(run==1391) return true; // st 0.67
	if(run==1387) return true; // st 0.40
	if(run==1526) return true; // st 2.00
	return false;
}
...
int D2_MMZ03kD_XP() {
	...
	int cut1[5] = {0,40,40,0,0};
	int cut2[5] = {0,0,5,0,0};
	...
	for(int id=0; id!=3; ++id) {
		...
		fit[id] = new TF1( Form("fit%d",id), "[0]+[1]*x" );
		//fit[id] = new TF1( Form("fit%d",id), "[0]+[1]*x+[2]*x*x" );
		fit[id]->SetLineColor(color[id]);
	}


	TH2D *axis = new TH2D("axis",";xpitch  (mm);resolution  (um)",100,0,3,100,0,maxY);
	axis->Draw();
	gr[0]->Fit(fit[0],"Q");
	gr[1]->Fit(fit[1],"Q");
	gr[2]->Fit(fit[2],"Q");
	...
	dummy->SetLineWidth(0);	TLegend *leg = new TLegend(0.15,0.70,0.60,0.87);
	leg->AddEntry( gr[0], "Straight pads" );
	leg->AddEntry( gr[1], "Zigzag: Period=0.5mm ; Stretch=0\%" );
	leg->AddEntry( gr[2], "Zigzag: Period=0.5mm ; Stretch=5\%" );

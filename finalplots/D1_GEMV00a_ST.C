...
int D1_GEMV00a_ST() {
	...
	TCanvas *main = new TCanvas("GEMV00a_STRETCH","");
	TH2D *axis = new TH2D("axis","GEM V00a 2mm xpitch ;stretch  (\%);resolution  (um)",100,-30,+30,100,0,maxY);
	axis->Draw();
	gr[0]->Fit(fit[0],"Q");
	gr[1]->Fit(fit[1],"Q");
	gr[2]->Fit(fit[2],"Q");
	...
	leg->AddEntry( gr[0], "1.0mm yperiod" );
	leg->AddEntry( gr[1], "0.8mm yperiod" );
	leg->AddEntry( gr[2], "0.5mm yperiod" );
	leg->AddEntry( gr[3], "0.4mm yperiod" );

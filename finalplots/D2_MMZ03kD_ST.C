...
int D2_MMZ03kD_ST() {
	...
	int cut1[5] = {40,40,40,0,0};
	double cut2[5] = {2,0.83,1,0,0};
	//int cut1[5] = {20,26,40,50,0};
	//int cut2[5] = {2,2,2,2,2};
	...
	TCanvas *main = new TCanvas("MMZ03kD_STRETCH","");
	TH2D *axis = new TH2D("axis","Micromegas Z03kD;stretch  (\%);resolution  (um)",100,-15,+25,100,0,maxY);
	axis->Draw();
	gr[0]->Fit(fit[0],"Q");
	gr[1]->Fit(fit[1],"Q");
	//gr[2]->Fit(fit[2],"Q");
	...
	leg->AddEntry( gr[0], "2.00mm xpitch ; 0.5 yperiod" );
	leg->AddEntry( gr[1], "0.83mm xpitch ; 0.5 yperiod" );
	//leg->AddEntry( gr[2], "yb=40 ; xp=1" );

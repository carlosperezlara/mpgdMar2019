...
bool filterKeyRuns(int run) {
	if(run==1374) return true;
	if(run==1377) return true;
	if(run==1375) return true;
	if(run==1379) return true;
	if(run==1392) return true;
	if(run==1389) return true;
	if(run==1391) return true;
	if(run==1387) return true;
	return false;
}
...
int D5_GEM_Z03kA_XP() {
	...
	for(int i=0; i!=8; ++i) {
		...
		rf[i].open( Form("testGX/D%dGX.csv",i+1) );
		nnn[i] = 0;
		for(;nnn[i]!=max;++nnn[i]) {
			rf[i] >> tgx[i][nnn[i]];
			if(!rf[i].good()) break;
	...
	for(int id=0; id!=4; ++id) {
		...
		for(int idx=0;idx!=nnn[bd];++idx) {
			if( (tyb[bd][idx]==cut1[id])
				...
				if(!goodrun(trn[bd][idx],bd,1)) continue;
	...
	//TH2D *axis = new TH2D("axis",";xpitch  (mm);resolution  (um)",100,0,2.2,100,0,maxY);
	TH2D *axis = new TH2D("axis",";xpitch  (mm);resolution  (um)",100,0,2.2,100,0,400);
	axis->Draw();
	gr[0]->Fit(fit[0],"Q");
	//gr[1]->Fit(fit[1],"Q");
    gr[2]->Fit(fit[2],"Q");
	...
	//dummy->SetLineWidth(0);	TLegend *leg = new TLegend(0.13,0.63,0.72,0.87,"GEM Z03kD");
	dummy->SetLineWidth(0);	TLegend *leg = new TLegend(0.13,0.63,0.72,0.87);
	leg->AddEntry( gr[0], "Straight pads" );
	//leg->AddEntry( gr[1], "Zigzag: yp=0.5 mm ; st=0\%" );
    leg->AddEntry( gr[2], "Zigzag: Period=0.5 mm ; Stretch=5\%" );
    //leg->AddEntry( gr[3], "Zigzag: yp=0.5 mm ; st=10\%" );

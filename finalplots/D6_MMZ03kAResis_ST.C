...
int D6_MMZ03kAResis_ST() {
	fillboardbad();
	gStyle->SetOptStat(0);
	const int max = 1000;
	const double maxY = 400;
	ifstream rf[8];
	int     nnn[8];
	double  tgx[8][max];
	double  tdx[8][max];
	...
	TH2D *axis = new TH2D("axis",";stretch  (\%);resolution  (um)",100,-30,+30,100,0,maxY);
	axis->Draw();
	//gr[0]->Fit(fit[0],"Q");
	gr[1]->Fit(fit[1],"Q");
	//gr[2]->Fit(fit[2],"Q");
	...
	TLegend *leg = new TLegend(0.25,0.67,0.43,0.87,"MMZ03kD");
	//leg->AddEntry( gr[0], "yb=40 ; xp=2" );
	leg->AddEntry( gr[1], "yb=40 ; xp=0.83" );

...
int D3_uRW_B01a_ST() {
	...
	int cut1[5] = {40,50,0,0,0};
	double cut2[5] = {2,2,0,0,0};
	...
	TCanvas *main = new TCanvas("uRWB01a_STRETCH","");
	TH2D *axis = new TH2D("axis","#muRWell B01a 2mm xpitch;stretch  (\%);resolution  (um)",100,-10,+25,100,0,maxY);
	axis->Draw();
	gr[0]->Fit(fit[0],"Q");
	gr[1]->Fit(fit[1],"Q");
	grC[0]->Fit(fitC[0],"Q");
